...
casia_dataset = CasiaIrisDataset("data/datasets")
...
for anchor, label, path in tqdm(casia_dataset, desc="Processing items", total=len(casia_dataset)):
    ...
    if output['error'] is None:
        ...
        file_path = f"{str(label_file).zfill(3)}/{eye_side}/{path.split('/')[-1].split('.')[0]}"
        labels_path.append((f"{label_file}-{eye_side}", file_path))

        # Save the cropped image
        write_img("data/datasets/CASIA-Iris-Thousand-Centered/" + file_path + ".jpg", get_cropped_iris_image(anchor, output["metadata"]["eye_centers"]["iris_center"]))
        # Save the normalized image
        write_img("data/datasets/CASIA-Iris-Thousand-Normalized/" + file_path + ".jpg", iris_pipeline.call_trace['normalization'].normalized_image)
        # save template to file
        write_template(output["iris_template"], "data/datasets/CASIA-Iris-Thousand-Encoding/" + file_path + ".json")


# Save the updated CSV file
updated_file = "data/datasets/iris_thousand_updated.csv"
